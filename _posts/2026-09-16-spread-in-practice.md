...
In the previous posts, we introduced [RL environments]({% post_url 2026-09-10-what-is-rl-environemnt %}). Here, we look at a concrete example where we built an RL env for a coding agent (model + harness) to build a music recommender system.

In this example, the task is to build a PyTorch two-tower model. One tower represents the listener, the other represents each track, and their dot product scores how well they match. We chose this constraint to reflect a realistic large-catalog retrieval setup that most companies face: track embeddings can be computed offline and stored in an approximate nearest-neighbor (ANN) index. At request time, the system computes the listener's embedding and retrieves tracks with high dot-product scores.

The environment provides listening histories for a set of users and a set of eligible tracks for each user. The agent has 60 minutes in an 8-vCPU, 8 GB container with no GPU, running on an AWS c6a.2xlarge instance. The agent can inspect the data, train models, and use a public validation dataset to choose its next experiment. Running the reference solution end-to-end takes about six minutes on that hardware, including data preparation, training, validation, ranking, and evaluation. This leaves room for several experiments, although agents also need time to write code and plan.
...
We tested three model/harness pairings, all at high reasoning effort: We ran five attempts with Opus 5.0 on Claude Code 2.1.251, ten attempts with GPT-5.6-Sol on Codex CLI 0.147.0, and five attempts with Grok 4.6 on Grok Build 1.0.5.
...
How much of this variation might come from randomness in model training? To establish a point of comparison, we ran the same reference implementation with ten different training seeds. Hidden NDCG@10 ranged from 0.0148 to 0.0212, with mean of 0.01766 a sample standard deviation of approximately 0.00217 ($$\sigma_r$$).

For each model/harness pairing, we also lookat the is the sample standard deviation of its final submission scores ($$\sigma_a$$). Following the [previous post]({% post_url 2026-09-13-what-makes-a-good-rl-task %}), we define spread as:
...
As shown in the above table, the agent scores varied substantially more than the fixed reference did across training seeds: their sample standard deviations were roughly two to four times as large. This suggest that the stronger attempt made better decisions and there is room to make the agent more robust for making such design and modeling decisions. In the next post, we will go deeper into the agent trajectories to study those decisions and identify areas of improvments.
